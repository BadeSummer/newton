...
# - SolverCoupledProxy performs lagged-impulse proxy coupling.
#
# Key Features:
# 1. Proxy Bodies: selected MuJoCo robot bodies are exposed in the VBD view
# 2. State Sync: handled by SolverCoupledProxy each substep
# 3. Force Harvesting: VBD proxy contact feedback is applied to MuJoCo
# 4. Force Subtraction: SolverCoupledProxy rewinds lagged feedback
# 5. Lagged Impulses: forces from substep k are applied at substep k+1
# 6. Virtual Inertia: SolverCoupledProxy installs proxy mass/inertia via hooks
#
# Command:
...
class Example:
    ...
    def _setup_shared_world_and_coupling(self):
        ...
        if not self.proxy_body_ids:
            raise RuntimeError("SolverCoupledProxy requires at least one proxy body")
    ...
    def simulate(self):
        ...
        SolverCoupledProxy owns source feedback, proxy state sync, velocity
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Number of SolverCoupledProxy relaxation iterations per coupled substep.",
    )
