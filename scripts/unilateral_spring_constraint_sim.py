...
"""Numerical simulator for a unilateral damped spring constraint.

Run directly:

    ./scripts/unilateral_spring_constraint_sim.py

The coordinate x is compression [m]. Contact is inactive for x <= 0.
The damping term is active only while compressed and still approaching.
"""
...
class UnilateralSpringApp(tk.Tk):
    """Tkinter UI for simulating a unilateral spring-damper constraint."""
    ...
    def __init__(self) -> None:
        super().__init__()
        self.title("Unilateral Spring Constraint Simulator")
    ...
    def _build_ui(self) -> None:
        ...
        specs = [
            ...
            ("force", "F_ext [N]", -200.0, 200.0, 10.0, 0.5),
            ("x0", "x(0) [m]", -0.05, 0.05, 0.0, 0.0005),
            ("v0", "v(0) [m/s]", -20.0, 20.0, 4.0, 0.1),
        ...
        ttk.Button(controls, text="Reset", command=self._reset).grid(row=3, column=1, sticky="e", padx=8, pady=(16, 0))
        ttk.Button(controls, text="Simulate", command=self._schedule_update).grid(
            row=3,
            column=2,
            sticky="e",
            padx=8,
    ...
    def _read_parameters(self) -> Parameters:
        ...
        return Parameters(
            ...
            stiffness=max(values["stiffness"], 0.0),
            force=values["force"],
            x0=values["x0"],
            v0=values["v0"],
    ...
    @staticmethod
    def _contact_force(params: Parameters, x: float, v: float) -> float:
        if x <= 0.0:
            return 0.0

        spring_force = params.stiffness * x
        damping_force = params.damping * v if v > 0.0 else 0.0
        return spring_force + damping_force

    def _simulate(self, params: Parameters) -> tuple[list[tuple[float, float, float]], SimulationInfo]:
        sample_count = min(max(2, math.ceil(params.t_max / params.time_step)), self._MAX_SAMPLES)
        dt = params.t_max / sample_count

        x = params.x0
        v = params.v0
        max_compression = max(x, 0.0)
        ...
        for i in range(sample_count + 1):
            t = dt * i
            contact_force = self._contact_force(params, x, v)
            points.append((t, x, contact_force))
            max_compression = max(max_compression, x)
            ...
            acceleration = (params.force - contact_force) / params.mass
            v += acceleration * dt
            x += v * dt

        active_fraction = active_steps / (sample_count + 1)
        return points, SimulationInfo(
            sample_count=sample_count,
            actual_time_step=dt,
            max_compression=max(max_compression, 0.0),
    ...
    def _update_plot(self) -> None:
        ...
        points, info = self._simulate(params)

        self.info_var.set(
            f"dt={info.actual_time_step:.4g} s    samples={info.sample_count + 1}    "
            f"max x={info.max_compression:.4g} m    max contact={info.max_contact_force:.4g} N    "
            f"final x={info.final_x:.4g} m    final v={info.final_v:.4g} m/s    "
            f"active={100.0 * info.active_fraction:.3g}%"
        )
        ...
        x_values = [x for _t, x, _force in points]
        y_min = min(min(x_values), params.x0, 0.0)
        y_max = max(max(x_values), params.x0, 0.0)
        ...
        zero_y = sy(0.0)
        self.canvas.create_line(left, zero_y, right, zero_y, fill="#475569", dash=(7, 5), width=2)
        ...
        self.canvas.create_text(18, top, text="x compression [m]", anchor="nw", fill="#1e293b", font=self._label_font)
